...
public class Enemy3 : Enemy
{
    ...
    public void pattern1()
    {
        ...
        now_dealing = turnManager.Turn * (N + 2);
        now_Shield = 0;
        Get_Shield(now_Shield);
        full_expain = "";
        full_expain += string.Format(dice_expain, N);
        full_expain += string.Format(attack_expain, "(현재 턴수)×(N+2)", now_dealing);
    }
    ...
    public void pattern2()
    {
        ...
        now_Shield = 0;
        Get_Shield(now_Shield);
        m_dice.Change_DiceVal(-3);
        full_expain = "";
        full_expain += string.Format(dice_expain, N);
        full_expain += string.Format(attack_expain, "(현재 턴수)×(N+1)", now_dealing);
        full_expain += "다음 주사위 값에 -3";
    }
}
